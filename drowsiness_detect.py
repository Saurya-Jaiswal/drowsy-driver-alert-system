...
EYE_CLOSED_FRAMES = 20  # Number of consecutive frames before alarm
COUNTER = 0  # Frame counter for closed eyes
ALARM_ON = False  # To track if alarm is playing
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        # Region of Interest (ROI) for eye detection (upper half of face)
        ...
        if len(eyes) == 0:  # No eyes detected (possibly closed eyes)
            ...
            if COUNTER >= EYE_CLOSED_FRAMES:
                if not ALARM_ON:  # Start alarm only if it's not already playing
                    pygame.mixer.music.play(-1)
                    ALARM_ON = True
                cv2.putText(frame, "DROWSINESS DETECTED!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
        else:
            COUNTER = 0  # Reset counter if eyes are detected
            if ALARM_ON:  # Stop alarm if it was playing
                pygame.mixer.music.stop()
                ALARM_ON = False
